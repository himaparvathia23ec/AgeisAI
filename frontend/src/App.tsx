    ...
    google?: {
      accounts: {
        id: {
          ...
          renderButton: (el: HTMLElement, options: { type?: string; theme?: string; size?: string; text?: string }) => void;
        };
      };
    };
  }
...
const LandingPage = ({ onGoogleLoginSuccess }: { onGoogleLoginSuccess: (idToken: string) => Promise<void> }) => {
  const [loading, setLoading] = useState(false);
  const [loginError, setLoginError] = useState<string | null>(null);
  const buttonRef = React.useRef<HTMLDivElement>(null);
  const callbackRef = React.useRef(onGoogleLoginSuccess);
  callbackRef.current = onGoogleLoginSuccess;
            ...
            <div className="glass-panel w-full max-w-md p-8 lg:p-10 rounded-2xl shadow-2xl flex flex-col gap-8 relative overflow-hidden group">
              ...
              <div className="flex flex-col gap-4 relative z-10">
                {GOOGLE_CLIENT_ID ? (
                  ...
                  <p className="text-amber-500 text-sm">Set VITE_GOOGLE_CLIENT_ID in frontend .env</p>
                )}
                {loading && (
                  <p className="text-center text-slate-400 text-sm">Signing in...</p>
                )}
                {loginError && (
...
export default function App() {
  ...
  const handleGoogleLoginSuccess = async (idToken: string) => {
    ...
    if (!data.success || !data.user_info) throw new Error('Invalid login response');
    ...
    <div className="font-display">
      <AnimatePresence mode="wait">
        {view === 'landing' && (
          ...
          >
            <LandingPage onGoogleLoginSuccess={handleGoogleLoginSuccess} />
          </motion.div>
        )}
